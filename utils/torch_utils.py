...
####def acc_score(y_true, y_pred):
    ...
    ####for pred, label in zip(y_pred, y_true):
        #print(pred, label)
        #print(np.sum(  (pred > 0.5) == (label > 0.5)  ))
        ####total += np.sum(  (pred > 0.5) == (label > 0.5)  ) / len(pred)   #预测标签
    ####return total / len(y_pred)
def acc_score(y_true, y_pred):
    total = 0
    dataset = "/home/nihao/nihaouser/old-disk/nihao/sentence_simi/dialog-HGAT-master1/"
    file_write = open(dataset + "result1.txt", "w", encoding = "utf-8")
    for pred, label in zip(y_pred, y_true):
        #print(np.argmax(pred), np.argmax(label), np.sum(  (pred > 0.5) == (label > 0.5)  ) , pred, label)
        #print(np.sum(  (pred > 0.5) == (label > 0.5)  ))
        #print(pred, label)
        #total += np.sum(  (pred > 0.5) == (label > 0.5)  ) / len(pred)   #预测标签
        file_write.write('%s	%s	%s	%s	%s'% (str(np.argmax(pred)), str(np.argmax(label)), str(pred), str(label), "\n"))
...
def f1_score(y_true, y_pred, T2=None):
    ...
    def geteval(devp, data):
        ...
        for i in range(len(data)):
            #print("len(data)len(data)len(data)len(data)", len(data))
            for id in data[i]:
                if id != 3:
                    correct_gt += 1
                    if id in devp[i]:
...
def f1c(devp, data):
    ...
    for i in range(len(data)):
        ...
        for k in range(len(data[i][1][j]["rid"])):
           if data[i][1][j]["rid"][k] != 3:
               t[data[i]["rid"][k]] = data[i]["t"][k].lower().strip()

        l = set(data[i]["rid"]) - set([3])
